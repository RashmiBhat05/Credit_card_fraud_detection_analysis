...
#load the dataset
creditcard_data <- read.csv("C:/Users/ADMIN/Downloads/creditcard.csv~/creditcard.csv")
...
pie(table(creditcard_data$Class), labels, col=c("Orange","red"),
    main ="Pie Chart")
...
#----------------------------------------
#decision tree
decisionTree_model <- rpart(Class ~ . , credit_smote)
...
decisionTree_model <- rpart(Class ~ . , sampled_credit)
predicted_val <- predict(decisionTree_model, test_data, type = 'class')
probability <- predict(decisionTree_model, test_data, type = 'prob')
rpart.plot(decisionTree_model)
confusionMatrix(predicted_val,test_data$Class)

decisionTree_model <- rpart(Class ~ . , train_data[,-1])
predicted_val <- predict(decisionTree_model, test_data[-1], type = 'class')
probability <- predict(decisionTree_model, test_data[-1], type = 'prob')
rpart.plot(decisionTree_model)
confusionMatrix(predicted_val,test_data$Class)
